...
public class HttpMgr : Singleton<HttpMgr>
{
    ...
    public void Download(string url, string saveFile, string tempFileName = null, Action cb = null, Action<double, double> downloadingCb = null)
    {
        MonoMgr.Instance.StartCoroutine(_download(url, saveFile, tempFileName, cb, downloadingCb));
    }
    IEnumerator _download(string url, string saveFile, string tempFileName = null, Action cb = null, Action<double, double> downloadingCb = null)
    {
        ...
        while (nReadSize > 0)
        {
            ...
            nReadSize = ns.Read(nbytes, 0, len);
            double dDownloadedLength = (fs.Length - lStartPos) * 1.0 / (1024 * 1024);
